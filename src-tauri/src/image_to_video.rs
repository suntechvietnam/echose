...
pub async fn create_video_from_images(
    ...
    // Với 4K, dùng medium preset và CRF 20 để cân bằng tốc độ và chất lượng
    let (preset, crf) = if video_quality == "4K" {
        ("medium", "20")  // Tối ưu cho 4K: nhanh hơn nhưng vẫn chất lượng tốt
    } else {
        ("medium", "18")  // 2K có thể dùng CRF thấp hơn một chút
    };
    
    // Tạo video từng ảnh với hiệu ứng - PARALLEL PROCESSING
    // Giới hạn số lượng concurrent tasks để tránh quá tải hệ thống
    // Với 4K, giới hạn 4-8 tasks đồng thời để tận dụng CPU nhưng không quá tải
    let max_concurrent = if video_quality == "4K" { 6 } else { 8 };
